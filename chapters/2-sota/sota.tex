...
The contents of different natures are related to each other to produce a story (e.g. speech text are related to speech balloon and speaking characters).
...
Fletcher and Katsuri~\cite{fletcher1988robust} are the reference in this domain which continues to be explored more than twenty years after for more and more complex documents~\cite{tombre2002text,Roy2007Text}.
From our knowledge, only one preliminary study have been proposed for text/graphic separation in comics~\cite{BernhauptWSES2001}.
This approach relies on artificial neural network, edge and corner detection.
The authors report results on very few panels and only for speech text, thus it can not be generalised to that we can found in comic book images (e.g. speech texts, onomatopoeias, captions).
...
Preliminary work about cartoon and comics faces recognition have been carried out by Kohei~\cite{Kohei2012}, Cheung~\cite{cheung2008face} and Bernhaupt~\cite{BernhauptWSES2001}.
...
In the next chapter, we are going to present a sequential content extraction approach that profit from the relations between elements to guide the retrieval process.
