...
The connected-component algorithm is applied on the binary image and the bounding box of each of the black component is computed to facilitate subsequent processing, see figure \ref{fig:se:cc_bounding_box}.
...
Their corresponds to ``panel'' (the highest), ``text'' (the most numerous) and ``noise'' (few pixels height) as shown on figure~\ref{fig:se:histo_roi}.
...
See figure \ref{fig:se:frame_candidates} and~\ref{fig:se:frame_cleaned}.
...
For this purpose, we defined two conditions that are auto-adaptive to each letter (see figure~\ref{fig:se:letter_position}):
...
One particularity of text inside balloons is its vertical and/or horizontal alignment in the balloon, see figure~\ref{fig:se:text_in_balloon}.
...
We propose to make a binary segmentation of the comics image and then use connected-component labelling method to extract white blobs, see figure~\ref{fig:se:closed_balloon}.
...
First, we require that the candidate text lines to group have similar heights (or width in case of vertical text) and second that the inter-line distance is smaller than the average text line height of the potential paragraph region, see figure~\ref{fig:se:line_to_paragraphs}.
...
See figure~\ref{fig:se:align_diff}.
...
Balloon contour is not always completely drawn, it may be implied by contrast difference or other surrounding elements (see fig.~\ref{fig:se:balloon_examples}c).
...
For the examples given in figure~\ref{fig:se:balloon_examples}, this would be a smooth contour with relatively constant curvature (fig.~\ref{fig:se:balloon_examples}a), an irregular one with high local curvature (fig.~\ref{fig:se:balloon_examples}b), and an implicit one with missing parts (fig.~\ref{fig:se:balloon_examples}c).
...
Here we perform edge detection using the well know Sobel operator (see figure~\ref{fig:se:distance_transform}).
The original Kass~\cite{Kass1988} external energy (see equation~\ref{eq:se:edge}) is appropriate for natural scene images with smooth gradients but not for stroke-based images such as comics that comprise uniform coloured regions with strong edges.
...
See figure~\ref{fig:se:paragraphs}b).
Note that the convex hull of the text area also corresponds to the $E_{text}$ maximal value border, see figure~\ref{fig:se:distance_transform}.
...
During the first low resolution localization step, we perform a spaced equipartition of the points (see figure~\ref{fig:se:paragraphs}c) to quickly localize the global shape avoiding unnecessary stops on image details.
...
%The and low external energy parameter $\gamma$, a large internal curvature energy parameter $\beta$ and a large knowledge energy parameter $\eta$ (see fig.~\ref{fig:se:multiscale01}). 
At the end of this step, we obtain a preliminary localization of the speech balloons, see fig.~\ref{fig:se:mono_res_det}. 
