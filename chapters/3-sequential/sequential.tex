...
From all the detected contours, we select the one $O$ that has the closest point $O_i$ from $v*$ (Euclidean distance).
Then we run through all the points of the sequence of points that defines the contour $O=\{o_0, o_1,...,o_n\}$ and define as tail tip position the one at the first maximum Euclidean distance to the tail origin.
The tail origin is a virtual point midway between $f_a$ and $f_b$ (Figure~\ref{fig:se:tail_tip_refinement}).
    ...
    \caption[Tail tip position detection]{Tail tip position detection. From left to right, the optimal vertex $v*$ in green and the local contour detection in blue, the closest point $O_i$ in red and its first stop at the maximal distance with the tail origin (dark green point).}
