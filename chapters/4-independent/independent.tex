...
Reaching such level of independence requires embedding more domain knowledge in each processing algorithm in order to give then the ability to autonomously process an image (without needing previous information extraction) with a similar performance to the sequential approach.
...
The main issue comes from the clustering algorithm that uses a fixed number of clusters which assumes that the image content connected-components from panels, text and noises (they are dependent from each other).
% Nevertheless, the panel extraction can be improved if we focus on panel elements only, especially in the case
This is not always the case or sometimes the amount of text or noise is not significant and confuses the clustering.
...
The advantage of the proposed processing sequence compared to other method from the literature is its ability to retrieve multi-script and multi-oriented text with complex background in an unsupervised way.
...
In practise, the foreground cluster often contains text plus other similar grey value regions (e.g. black strokes from contours) and the second one other pixels with a different grey values (e.g. bright graphic regions).
...
Instead of using a fixed threshold, we use the well-known Otsu's threshold selection method that has been extensively use since decades for this purpose but other methods would be appropriate as well (Appendix~\ref{sub:ap:bi_threshold}).
...
In additions, it should be robust to text-less pages which may appear randomly within some albums.
...
The text line are composed by 12 characters in average which give a good chances to OCR to recognize at least one of them (Figure~\ref{fig:ex:textline_lenth_distribution}).
...
Instead of relying on the text detection we propose to keep both processes independent in order to avoid propagation of errors.
...
It only works using a empirical approach for choosing the threshold value and is appropriate for comics from the same style and digitized under the same light condition.
...
The well-known Otsu's threshold selection method~\cite{otsu79} have been proposed for this purpose, we use it to define the optimal threshold and apply it on the whole image in order to make a global binary segmentation of the grey-scale image (Appendix~\ref{sub:ap:bi_threshold}).
...
Note that bi-level segmentation (binarisation) creates a lot of small connected-components on textured regions; they are not disturbing the process as we are interested by balloons which are quite big regions.
We prune small connected-component using the clustering algorithm presented Section~\ref{sec:se:panel_and_text} (ignores the connected-components that are part of the cluster with the lowest average height).
...
For instance, Figure~\ref{fig:in:contour_style} shows six different speech balloon comics styles where we can note the difficulty to assign one class for each balloon without knowing others balloon shapes in the image and reading the contained text.
...
There are no previous approaches in the literature, we briefly reviewed state of the art approaches concerning shape and contour classification (which are the closest fields of research) but none of them match our specific issue (Section~\ref{sec:sota:balloon_segmentation}).
...
Note that this approach requires balloon to be extracted at pixel-level such as proposed in Sections~\ref{sub:se:regular_balloon_extraction} and~\ref{sub:in:balloon_segmentation}.
...
The shape (e.g. rectangle, square, circle and oval) does not provide a lot of information about how the text is spoken, it is more related to the style of the comics and the structure of the panel.
Note that shape descriptors
However, the contour variation transcribes the speech tone information according to the different patterns which compose the contour (e.g. straight lines, curves, peaks, half circles).
...
In fact, they are no texture in many comics because they are designed with colour flats (see comics design process Section~\ref{sec:document_image_analysis}), nevertheless, the digitization process sometimes creates textures (e.g dithering, compression).
...
Comics have a particular structure that allows different approaches for object retrieval. An interesting point is that same instances of objects often appears in different panels and therefore several times in each page of the album.
...
This method is experimented and discussed Section~\ref{sub:ex:text_extraction_recognition_evaluation}.
...
These models allows consistency analysis of extracted information and inference of the relations between all the extracted elements such as the reading order, the type of text (e.g. spoken, onomatopoeic, illustrative) and the relations between the speech balloons and the speaking characters.
