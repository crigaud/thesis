...
The advantage of the proposed processing sequence compared to other method from the literature is its ability to retrieve multi-script and multi-oriented text with complex background in a unsupervised way.
...
Several approaches exist and their performance rely on the input data distribution.
...
We propose to relax the constraint by using an binary segmentation method that automatically selects the optimal threshold value.
...
However, the contour variation transcribe the speech tone information according to the different patterns which compose the contour (e.g. straight lines, curves, peaks, half circles).
...
Contour description aims to encodes discriminant contour variations in order to facilitate the classification step.
...
The first step consist in choosing a proper colour palette and the second step is to reconstruct an image by replacing original colours with the most similar colour from the palette.
...
The fact do not include any spatial or quantitative information (e.g. region positions, number of pixels) allows the method to be scale and deformation invariant.
...
The optimal situation is when a colour appears a lot in the region of the character and only in this region, this would facilitates the character spotting process.
...
This high level approach is also used for understanding the image content and retrieve the semantic relations between elements (context).
