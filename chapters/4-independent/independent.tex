...
\begin{figure}	%trim=l b r t  width=0.5\textwidth, 
  \centering
	...
	\hspace{1em}
	\subfloat[Binary segmentation]{\label{fig:se:panel_binary}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=0.2\textwidth]{panel_binary.png}}
	\hspace{1em}
...
% 	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\begin{figure}[h!]	%trim=l b r t  width=0.5\textwidth,
	  \centering
		...
		\caption[Different threshold selection applied on a grey-scale image]{Different threshold selection methods applied on the luminance channel of several comics types. First row shows the original image of a selection of several comic book parts. Second row shows the corresponding luminance channel of HSL colour space in a 8 bits grey-scale image. The third row represents the bi-level segmentation with a fixed threshold at 250 (as Arai~\cite{Arai11}) and fourth row shows the corresponding results using the automatic Otsu's threshold selection method.}
...
Here we use the same method presented in Section~\ref{par:se:letter_to_line} to group text component into text lines considering only the text height (or width for vertical text).
...
The text line are composed by 12 character in average which give a good chances to OCR to recognize at least on of them (Section~\ref{sec:dataset_description}).
...
\label{sub:in:text_conclusion}
The proposed approach consist in a adapted bi-level segmentation followed by a three rules filtering based on contrast, topology and local similarities to classify the connected components as being part of either graphics or text region.
A text line level verification valid the textual content of the candidate regions using an OCR system.
...
In this section we present a method for pixel level balloon extraction based on connected-component content analysis.
This method does not requires previous text extraction unlike the method presented Section~\ref{sub:se:regular_balloon_extraction}.
...
Contour classification is performed according to the selected contour descriptor (Section~\ref{sec:be:description}) by using a naive Bayesian Classifier~\cite{Mitchell1997} \modif{TODO: link to method description in appendix?}.
