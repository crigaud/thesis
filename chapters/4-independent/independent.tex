...
This approach tries to fill the weaknesses of the sequential approach presented in the previous chapter by making each specific content extraction algorithm independent from each other in order to avoiding error propagation.
...
The first approach presented in \ch{chap:sequential} is very specific to images from comic books because each extraction of element requires one or several pre-processing (other element extractions).
...
Independent panel, text and balloon extraction methods are proposed, additionally, text recognition and balloon classification complete the amount of information that is retrieved from the image content, towards a semantic description.
...
It is a two-step analysis that first generates text lines using ``a set of very rigorous rules'' to test each pair of connected-components and decide if they are candidate text regions or not.
...
The genericity is always a challenge is document image analysis systems, especially because it requires an additional effort for researchers to validate their methods on several other application domains (Section~\ref{sec:document_image_analysis}).
...
From the reviewed methods Section~\ref{sec:sota:balloon_segmentation}, we can see that the text position often helps in the localization and then segmentation of speech balloons.
The issue in relying on text extraction is that we become dependent on its performance.
Text extraction in comics images is still a challenging problem (Section~\ref{sec:in:text_localisation_and_recognition}), it is a very hard assumption to assume that text extraction will work perfectly. 
...
The well-known Otsu's threshold selection method~\cite{otsu79} have been proposed to find the best threshold that separate clear from dark regions.
...
Therefore, we propose a new approach that separates contour from shape information, analyse the contour variations and then, describes and classifies the balloon contours into three classes (see ``smooth'', ``wavy'' and ``spiky'' contour types in Figure~\ref{fig:sota:balloon_examples}). 
...
In this study, we describe the contour signal $c$ by using a variance-based two dimensional descriptor, more precisely the standard deviation $\sigma$.
...
For instance, if a contour has a higher standard deviation in the below signal part than the above signal part (according to the average $\bar{c}$) then the contour has more peaks in the direction of its barycentre than the opposite, which is not a characteristic of the ``smooth'' but ``wavy'' type (half circles).
...
\end{equation}
where $f_2$ is the feature two, $\sigma_{pos}$ and $\sigma_{neg}$ are the standard deviations of the signals $c_{pos}$ and $c_{neg}$ respectively.
...
In fact, they are no textures in many newer comics because they are designed and reproduced with colour flats (see comics design process Section~\ref{sec:document_image_analysis}), nevertheless, the digitization process sometimes creates textures (e.g dithering, compression).
...
The colour subset $D$ is composed by the $N$ colours most represented in the query region and that are the most discriminative in the image or collection.
...
Each window position is computed with 50\% per cent overlapping with its four neighbouring windows.
...
We define the detection confidence $C_{w_i}$ according to the number of identical colours between the query descriptor $D$ and each sliding window position $W=\{w_0,w_1,...,w_n\}$ (Equation.~\ref{eq:in:confidence_character_spotting}).
