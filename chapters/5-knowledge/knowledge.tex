...
%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
  ...
  \caption[Percentage of panels, balloons, text lines and characters from the eBDtheque dataset that fit the proposed model]{Percentage of panels, balloons, text lines and characters from the eBDtheque dataset~\ref{sec:dataset_and_ground_truth_construction} that fit the definition given in \ref{sec:kn:knowledge_representation} according to the area proportion that need to be covered.}
...
\label{par:step_5}
The expert system checks if the spatial relations of the characters $C$ match the properties of the knowledge base defined in Section~\ref{sec:kn:knowledge_representation} (Figure~\ref{fig:kn:valid_2}).
...
Then, each element was linked to its smallest container in a half-blind way (Section~\ref{sec:kn:knowledge_representation}).
...
\modif{TODO: ROI same as Method 1, reading order (SOTA), other from Clement's thesis (coming soon)}
