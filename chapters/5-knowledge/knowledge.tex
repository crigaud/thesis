...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
  \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.7\textwidth]{expert_system.pdf}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
  \includegraphics[trim= 140px 350px 100px 95px, clip, width=0.9\textwidth]{process_loop.pdf}
  \caption{Process loop of the framework.}
...
The term ``pointed'' refers to the fact that the character is included in the part of the panel indicated by the tail. To have a better understanding of how a panel is divided according to tail direction, please refer to Section~\ref{sec:se:from_tail_to_comic_character}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
   \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.6\textwidth]{process_illustration_hypo_1.png}\\
...
\label{par:step_2}
Subsequently, the expert system checks if their spatial relations (context) match the topological properties of the knowledge base defined in section~\ref{sub:kn:knowledge_base}, otherwise it solves them using the domain knowledge described in Section~\ref{sub:kn:validation}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
   \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.6\textwidth]{process_illustration_valid_1.png}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
   \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.6\textwidth]{process_illustration_hypo_2_1.png}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
   \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.6\textwidth]{process_illustration_hypo_2_2.png}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
   \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.6\textwidth]{process_illustration_valid_2.png}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[!ht]  %trim=l b r t  width=0.5\textwidth,
   \centering
   \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.6\textwidth]{process_illustration_infer_2.png}\\
...
Further iteration could be processed by extracting other low level elements such as faces or vehicles and by adding extra knowledge in the knowledge base.
...
Our knowledge base is composed of two ontologies, designed using OWL's W3C recommendation~\cite{McGuinness2004}, and interacting with each other (Figure~\ref{fig:kn:generic_expert_system}).
...
Therefore, the ROI concept is linked to the Image and Extractor concepts respectively with functional properties \textit{hasImage} and \textit{hasExtractor}.
...
Figure~\ref{fig:kn:model_image} shows a visual representation of these concepts altogether.
 ...
 \begin{figure}[!ht]
   \centering
  \includegraphics[width=120px]{model_image.pdf}
...
Album is linked to Page and Page to Content respectively by the \textit{hasPage} and \textit{hasContent} properties.
The concept of Content stands for any kind of visual element that can appear on an analysed page of comic book.
It is specialized into the concepts of \textit{Panel}, \textit{Balloon}, \textit{TextLine} and \textit{Character}.
As it was stated in Section~\ref{sub:kn:knowledge_base}, we consider that panels are only contained by a page, characters and balloons by panels and lines of text by balloons.
...
The concepts of \textit{Speaker}, \textit{SpeechBalloon} and \textit{SpokenTextLine} are subsumed by \textit{Character}, \textit{Balloon} and \textit{TextLine} respectively and are derived with an OWL translation of the rules defined in ~\ref{sub:kn:knowledge_base}.
This hierarchy is visually represented in Figure~\ref{fig:kn:model_image}.
Balloons and text lines are also enriched by some attributes, like the direction indicated by the tail (\textit{tailDirection}) or the textual transcription (\textit{hasText}).

 \begin{figure}[!ht]
   \centering
  \includegraphics[width=230px]{model_comics.pdf}
...
This section presents the low and high level processing interaction to validate, infer information and generate hypotheses about image content.% we use it to validate the extracted elements, infer more information about them and express region of interest's hypothesis to send back at low-level algorithms.
\subsection{Validation of the extractions} % (fold)
\label{sub:kn:validation}
In order to validate the extraction of the comic page's components, we made sure that the extracted panels, balloons and text lines were in accordance with the knowledge defined in \ref{sub:kn:knowledge_base}.
...
Then, each element was linked to its smallest container (as defined in \ref{sub:kn:knowledge_base}) in a half-blind way.
...
In Section~\ref{sub:kn:knowledge_base}, we defined a speech balloon as a balloon that has a tail.
...
Among the validated characters (see~\ref{sub:kn:validation}), we considered as being potential speakers those who intersect with the hypothetical region computed in \ref{sec:se:from_tail_to_comic_character}.
