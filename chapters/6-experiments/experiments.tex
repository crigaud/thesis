...
Table~\ref{tab:panel_extraction_comparision_results} compare the average results we obtained for the three proposed methods and two methods from the literature~\cite{Arai11}.
...
We compare our results to Arai~\cite{Arai10} and Ho~\cite{Ho2012} that are two state of the art methods (Section~\ref{sec:sota:layout_panel}).
The first one use connected-component analysis similarly to our proposition and the second is based on growing region.
The performed the evaluation on the 1092 balloons of the eBDtheque dataset~\cite{Guerin2013} at object (Section~\ref{sub:ex:object_segmentation_metric}) and pixel levels in order to provide a performance evaluation for localisation and contour analysis purposes.
...
In Section~\ref{sec:se:from_text_to_balloon} we presented two sequential approaches for regular and implicit balloon extraction from text line positions.
Here, we evaluated them separately and detail the benefits of of energies that compose the implicit balloon extraction.

Regular balloon extraction method attributes a confidence value $C_{balloon}$ to each balloon candidate according to the alignment of the text paragraph it contains.
...
The implicit balloon extraction method is evaluated using two different scenarios in order to highlight the benefits of both active contour theory and domain specific knowledge.
% that considers as balloon any white connected components that overlaps at more than $10\%$ with text regions.
First, the active contour with the distance transform based external energy described section~\ref{sec:se:external_energie} (Table~\ref{tab:ex:bal_loc}, Method A (2)) and second, we add the domain knowledge energy $E_{text}$ from Section~\ref{sec:se:text_energie} (Table~\ref{tab:ex:bal_loc}, Method A (3)).
% Fourth, after a two stage contour fitting (4).
% Results are presented in table~\ref{tab:ex:bal_loc}.


For both regular and implicit balloon extraction of Method A, the input text is given from the sequential text extraction method (Section~\ref{sec:se:panel_and_text}) as it is the previous step of this approach.
...
The evaluation results are presented in Table~\ref{tab:ex:bal_loc} at object bounding box level (Section~\ref{sub:ex:object_localisation_metric}) and then in Table~\ref{tab:ex:high_res} at the level of pixel (Section~\ref{sub:ex:object_segmentation_metric}) to illustrate the level of detail of the segmentation.


+ Sequential approaches such as Arai, Ho and method A search for balloon inside the panel region, as a consequence, these methods can not extract balloons that overlap several panels.
...
Object localisation are usually performed at the level of object bounding box.
...
% \extrarowheight as needed to properly center the text within the cells
  ...
  \hline
  Method A (3)    & \modif{83.4}  & \modif{55.5}    & \modif{66.6}  \\
  % \hline
...
The baseline Method A (1) detects less than half of the balloons (about $37\%$ recall) of this dataset.
This approach is not originally designed for detecting implicit balloons but knowing the position of the panels allows masking non panel regions that sometimes recover implicit balloons (Figure~\ref{}).
...
First, as the a connected component is considered as balloon, when correct, the precision at the pixel level is maximal.
Second, it is faster to compute.
The results using active contour with distance transform Method A (2) shows a significant improvement, thanks to the active contour theory that detect much more balloons whether open or closed than connected component based methods. 
...
The independent balloon extraction Method C is TODO...
...
As mention Section~\ref{??}, the balloon contour contains part of information that is also important to retrieve in the context of understanding comic book documents (e.g. tail, tone information).
...
%     \centering
%     \caption[Pixel-level balloon segmentation evaluation scores]{Pixel-level balloon segmentation evaluation scores.}
...
%   Ho~\cite{Ho2012}  & ???     & ???   & & ???     & ???&    \\
  ...
  Arai~\cite{Arai11}     & \modif{0.8}       & \modif{0.8}     & \modif{???}       & \modif{???}  & \modif{???} & \modif{???}   \\
  \hline
  Method A (1)     & \modif{56.6}     & \modif{79.2}   & \modif{66.0}  & \modif{53.1}     & \modif{53.0} & \modif{53.1}    \\
%   \hline
%   (2) & ???     & ???   & & ???     & ???&    \\
  %\hline
  %Log dist. trans.       & ???       & ???   & & ???       & ???   \\  
...
%   (2)     & ??? & ??? & ???   & ??? & ??? & ??? \\
  \hline
  Method A (2)   & \modif{89.0}  & \modif{90.7}   & \modif{89.8}  & \modif{82.1}  & \modif{53.7}    & \modif{64.9}  \\
  \hline
  Method A (3)   & \modif{92.3}  & \modif{94.4}  & \modif{93.4}  & \modif{83.4}  & \modif{55.5}    & \modif{66.6}  \\
  \hline
  Method A (4)   & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}    & \modif{?}  \\
  \hline
  Method B   & \modif{57.9}  & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}    & \modif{?}  \\
  \hline
  Method C   & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}    & \modif{?}  \\
  \hline
...
An example of the Gaussian distribution parameters of the training set presented Figure~\ref{fig:ex:balloon_training_set} is given table~\ref{tab:in:coefficient}.
