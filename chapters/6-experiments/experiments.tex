...
The automatic extraction method is the best method from the summary Table~\ref{tab:ex:balloon_localisation_comparison_results} and the manual is simply the balloon contour from the eBDtheque dataset.
...
We evaluated the detection of comic characters on the 1550 characters of the eBDtheque dataset.
...
We compare the performance of this method given the input information of panel, balloon and tail from the ground truth and from the best automatic extraction method presented Table~\ref{tab:panel_extraction_comparision_results},~\ref{tab:ex:balloon_localisation_comparison_results} and~\ref{tab:ex:tail_evaluation} respectively.

We obtained average recall and precision scores of \modif{??}\% and \modif{??}\% (Figure~\ref{fig:ex:character_methodA_extraction_detail}).
...
If we focus on the subset of 880 speakers, the recall, precision and f-measure were 34.49\%, 32.02\% and 33.21\% respectively for Method C.
...
The experiments show interesting detection results based on a \modif{user-defined colours query descriptor} (Section~\ref{sec:proposed_method}). 
