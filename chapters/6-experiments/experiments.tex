...
We describe the metrics we use to evaluate our each contribution and finally compare our work to previous approaches from the literature.
...
\begin{figure}[ht]  %trim=l b r t  width=0.5\textwidth,
  ...
  \caption{Examples of comic panels that reflect the diversity of comic books of the dataset.}
...
The text lines contains 12 elements in average (Figure~\ref{fig:ex:textline_lenth_distribution}) and there are more than hundred text lines that are composed by only one letter which are punctuation or single letter words such as ``I'' or ``A''.
...
TODO describe metadata attribute \emph{relatedTo}
...
We evaluated the new tail extraction method on the 1092 balloons of the eBDtheque dataset~\cite{Guerin2013} ``version 2014'' and both regular and implicit balloon extraction methods presented Section~\ref{sec:se:from_text_to_balloon}.
