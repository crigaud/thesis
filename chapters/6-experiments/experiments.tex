...
We describe the metrics we use to evaluate each contribution (method 1, method 2 and method 3).
The evaluation of method 3 is decomposed in two parts: the validation and the inference processes.
Validation are combined with method 1 and 2 in each extraction evaluation to show its benefits.
The inference part can not be directly compared to low level information extraction, hence we discuss it in a proper section (Section~\ref{sec:semantic_links_evaluation}).
...
\modif{TODO describe metadata attribute \emph{idBalloon} in Line elements and \emph{idCharacter} in Character element}
...
\modif{TODO (conclude the dataset + ground truth presentations)}
...
TODO for pixel-level balloon extraction section?
...
We chose to compare our results to Arai~\cite{Arai10} and Ho~\cite{Ho2012} that are two methods from the state of the art (Section~\ref{sec:sota:layout_panel}).
The first one use connected component analysis similarly to our proposition and the second differs because it is based on growing region that does not require binary segmentation.

The comparative evaluation is performed on the 850 panels of the eBDtheque dataset~\cite{Guerin2013} ``version 2014'' at bounding box level, and using the recall and precision metrics introduced Section~\ref{sec:ex:metrics}.
...
\subsubsection{Arai's method~\cite{Arai10}} % (fold)
% \label{sub:arai_cite_arai}
We re implemented the comic panel extraction method presented in~\cite{Arai10} except not the division line detection by lake of detail in the original paper.
...
Single panel strips are not so common and would not have affected the result significantly.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.8\textwidth]{division_line_detection_arai10.png}
\caption[Division line detection from~\cite{Arai10}]{Division line detection from~\cite{Arai10} (figure 5 in the author' paper).}
...
\subsubsection{Ho's method~\cite{Ho2012}} % (fold)
We re implemented the Ho's method~\cite{Ho2012} with the original parameters and set the minimal and maximal lower brightness difference of the growing region method to 20, which was no mentioned in the original paper.
...
Note that the method also extract text region at the same time (Section~\ref{sub:ex:text_extraction_recognition_evaluation}) which does not interfere with panel.
...
This parameter avoid considering small and isolated elements (e.g text, logo, page number) as panel.
...
It valid or reject panel candidates using the proposed model (Section~\ref{sub:kn:validation}).
...
Table~\ref{tab:panel} presents the average results we obtained for the three proposed methods and a method from the literature~\cite{Arai11}.
...
This over segment the image that include some text of of panel or in implicit balloons \modif{TODO: to illustrate}.
...
The first proposed panel extraction (Method A) based on connected component clustering is simple to implement, and is a fast and efficient method for comics with disconnected panels (separated by a white gutter).
...
\begin{figure}
 ...
 \caption{Panel extraction score details for each image of the eBDtheque dataset \modif{TODO replace for table?}.
 }
...
\section{Text extraction and recognition evaluation} % (fold)
...
Also, a text recognition evaluation is given as baseline.
...
\subsubsection[Arai's method]{Arai~\cite{Arai11}} % (fold)
This method is a sequential approach that requires panel and balloon extraction presented in Section~\ref{sub:ex:panel_extraction_evaluation} and~\ref{sub:ex:balloon_extraction_evaluation} respectively.

TODO: detail the method here?

This approach was first developed for Japanese manga text extraction but we adapt it to also handle horizontal text.
Its adaptation to horizontal text consist in switching width and height related parameters of the concerned elements throughout the method.
Also, the kernel used for mathematical morphology is rotated of 90$^{\circ}$.


\subsubsection{Method A} % (fold)
% \label{sub:method_1}
The method proposed Section~\ref{sec:se:panel_and_text} does not require any particular parameter, only the number of cluster has to be fixed $k=3$ for the k-mean clustering.
...
As shown in Table~\ref{tab:ex:bal_loc}, balloon extraction is very low using this approach thus, it narrows down the text extraction in turn.
...
We evaluate the three approaches on the 1092 balloons of the eBDtheque dataset~\cite{Guerin2013} ``version 2014'' at object bounding box and pixel levels, and compare to previous work from the literature.
...
\subsubsection{Arai's method~\cite{Arai11}} % (fold)
% \label{sub:arai_s_method_cite}

TODO: describe the method here or in intro contribution justification?
Arai's method is a sequential approach that search for panel first and then balloons inside panels.
Customization: vertical AND horizontal text adaptation + adaptive thresholding
