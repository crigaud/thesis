...
The comic characters extraction are presented in Table~\ref{tab:ex:eval_characters} for Method A and C.
Method B is evaluated separately on a smaller subset of the dataset.
Note that the scores in Table~\ref{tab:ex:eval_characters} are given for the whole dataset including the characters that are not speaking.
...
If we focus on the subset of 880 speaking characters, the recall, precision and f-measure were \modif{34.49\%}, \modif{32.02\%} and \modif{33.21\%} respectively.
%First, the limitation of the extractor to process speaking characters; 
...
The experiments shown interesting detection results based on a \modif{user-defined colours query descriptor} (Section~\ref{sec:proposed_method}).
...
In the same way, of the 880 \emph{isSaidBy} relations, that link speech balloons to speaking characters, 9.5\% were undetectable because they were generated from balloons outside the panel.
