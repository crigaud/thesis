...
We describe the metrics we used to evaluate each contribution (method 1, 2 and 3).
The evaluation of method 3 is decomposed in two parts: the validation and the inference processes.
Validations are combined with method 1 and 2 in each extraction evaluation to show its benefits.
The inference part of method 3 can not be directly compared to low level information extraction, hence we discuss it in a proper section (Section~\ref{sec:semantic_links_evaluation}).
...
The best score of \modif{$R=??$} and \modif{$P=??$} was obtained for \modif{$C_{balloon} > 10\%$}.
...
The best score of \modif{$R=??$} and \modif{$P=??$} was obtained for \modif{$C_{balloon} > 10\%$}, details are given Figure~\ref{}.
...
\begin{figure}[h]
 ...
 \label{fig:ex:balloon_extraction_methodB_regular}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Figure~\ref{fig:ex:balloon_extraction_methodB_regular} confirms that our method works best when the balloons are closed, well segmented and with non cursive text inside.
...
In this sub-section, we will discuss balloon localisation at bounding box and pixel-level, and balloon classification.
...
The post validation by the expert system improved the precision but decreased the recall of the extraction while improving the overall f-measure by almost 3\% (Method 3).
...
\subsubsection{Method A} % (fold)
% \label{sub:method_a}
TODO: regular + implicit
% subsection method_a (end)

\subsubsection{Method B} % (fold)
% \label{sub:method_b}
TODO: regular
% subsection method_b (end)
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          Method A (1)  & \modif{??}    & \modif{??}    & \modif{??}    \\
          \hline
          Method A (2)  & \modif{??}    & \modif{??}    & \modif{??}    \\
          \hline
          Method B      & \modif{??}    & \modif{??}    & \modif{??}    \\
          \hline
...
Therefore, we do not expect to reach 100\% recall and precision with Method A because is able to detect only speaking characters.
Method B is potentially able to retrieve all the character instance given an example.

% \subsubsection{Sequential approach} % (fold)
% \label{sub:sequential_approach}

% subsection sequential_approach (end)
Similarly to the other extractions, the overlapping ratio (Section~\ref{sub:ex:object_localisation_metric}) between a predicted character region and a region from the ground truth should be above 50\% ($a_0>0.5$) for the predicted region to be considered as valid (true positive).
An example of valid and rejected regions is shown on Figure~\ref{fig:ex:characters_overlap_ratio}.
...
Method B consist in an independent comic character instance spotting given an example.
