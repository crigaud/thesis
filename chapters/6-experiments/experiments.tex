...
We describe the metrics we used to evaluate each contribution (Method A, B and C).
...
An example of ground truth file is given Listing~\ref{list:svg}.% présente un exemple du contenu de l'un de ces fichiers.
...
\subsection{Object segmentation} % (fold)
...
We evaluated the semantic relations $STSB$ and $SBSC$ according to the metadata in the ground truth of the eBDtheque dataset~\cite{Guerin2013} called \texttt{{isLineOf}} and \texttt{{isSaidBy}}, which represent 3427 and 880 relations respectively.
...
All the evaluation are performed on the 850 panels of the eBDtheque dataset~\ref{sec:dataset_and_ground_truth_construction} at object bounding box level, using the recall and precision metrics introduced Section~\ref{sub:ex:object_localisation_metric}.
...
\subsection{Arai's method~\cite{Arai10}} % (fold)
...
\caption[Division line detection from~\cite{Arai10}]{Division line detection from~\cite{Arai10} (figure 5 in the author's paper).}
\label{fig:ex:division_line_detection}
\end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The score of this method was in average for recall and precision of 20.00\% and 18.75\%.
...
\begin{figure}[!ht] %trim=l b r t  width=0.5\textwidth, 
  ...
  \subfloat[After erosion step]{\label{fig:ex:panel_ho_erode}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=0.2\textwidth]{ho04.png}}  
    \caption[Panel extraction process of Ho~\cite{Ho2012}]{Panel extraction and separation process of Ho~\cite{Ho2012}.}
    \label{fig:ex:panel_ho_extraction_process}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The score of this method was in average for recall and precision of 49.76\% and 68.03\% (Figure~\ref{fig:ex:panel_ho_extraction_detail}).
...
The score of this method was in average for recall and precision of 60.59\% and 82.51\% (Figure~\ref{fig:ex:panel_methodB_extraction_detail}).
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          Ho~\cite{Ho2012}   & 49.76       & 68.03    & 57.48    \\
          \hline
          Method A          & 64.24       & 83.81   & \bf{72.72}     \\
          \hline
          Method B          & 60.59       & 82.51   & 69.87      \\
          \hline
          Mtehod C   & ?     & ?     & ?      \\
          \hline
        \end{tabular}
...
\begin{figure}[h]
 ...
 \caption{Sample of Text Extraction Process. (a) Original balloon text image; (b) Threshold; (c) Morphology-Erosion filter; (d) Morphology-Opening filter; (e) Extracted blob from original image. Image from the original paper~\cite{Arai11}.
 }
...
The score of this method was in average for recall and precision of 2.81\% and 1.63\%.
...
An validation on the eBDtheque dataset shown that the best Mahalanobis' distance is \modif{???}.
...
Text line recognition is used as last filtering operation to validate the presence of text inside the candidate regions, it improves the result quality about \modif{2.00\%} and \modif{20.00\%} for recall and precision respectively.
  ...
  %   % \normalsize
  %   \centering
  %   \caption{Score for text localisation using Method B.}
  %   \begin{tabular}{|c|c|c|c|}
  %         \hline
  %          &  $R$ (\%)  & $P$ (\%)  & $F$ (\%)  \\
  %         \hline
  %         Not using OCR  & \modif{??}   & \modif{??} & \modif{??}       \\
  %         \hline
  %         Using OCR  &  \modif{59.92\%}  & \modif{46.26\%}     & \modif{??}       \\
  %         \hline
  %       \end{tabular}
  %   \label{tab:ex:ocr_impact_text_localisation}
  % \end{table}%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The score of this method was in average for recall and precision of \modif{59.92\%} and \modif{46.26\%} (Figure~\ref{fig:ex:text_methodB_extraction_detail}).
...
Considering the best method for panel extraction (\modif{???} in Figure~\ref{tab:panel_extraction_comparision_results}), the score of this method was in average for recall and precision of \modif{??}\% and \modif{??}\% (Figure~\ref{fig:ex:text_methodC_extraction_detail}).
...
\modif{TODO}
  ...
  \begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          Method A  & 54.68   & 56.92    & 55.78       \\
          \hline
          Method B  & 59.92   & 46.26    & 52.21       \\
          \hline
...
As shown in Table~\ref{tab:ex:bal_loc}, balloon extraction is very low using this approach thus, it narrows down the text extraction in turn (Table~\ref{tab:text_results}).
Moreover, this text extraction method uses several thresholds which reduces its fields of application.


% In our previous work~\cite{Rigaud2013VISAPP} text extraction was evaluated on a subset of 20 pages of the eBDtheque dataset~\cite{Guerin2013}.
% Here we applied it to the whole dataset.
Method A improve by \modif{???}\% the recall thanks to its genericity, moreover, even if the panel extraction is performed at the same time, it does not bias the text extraction because text areas are extracted from the whole page and not from panel regions.
Method B raise the precision by \modif{20}\% when using an OCR-based filter, without a significant loss in recall.
...
Table~\ref{tab:ex:text_recogniton_results} shows text recognition results given the text line position from the ground truth (best reachable score) and from the best automatic text localisation method (\modif{Method B} in Figure~\ref{tab:text_results}).
  ...
  \begin{table}[ht]
    % \normalsize
    \centering
    \caption{Text recognition results from ground truth and automatic text line localisation with an edit distance ($ED$) of 0, 1 and 2.}
    \begin{tabular}{|c|c|c|c|}
          \hline
...
We obtained a score of $A_{textReco}=$\modif{7.18\%} from the automatic text extraction method which constitute a baseline for future work on text recognition on the eBDtheque dataset~\cite{Guerin2013}.
We performed a more relaxed evaluation where we also considered as correct the text lines at a text edit distance equal to one, the accuracy rise to \modif{10.46\%}.
...
We compare our results to Arai~\cite{Arai10} and Ho~\cite{Ho2012} that are two state of the art methods (Section~\ref{sec:sota:layout_panel}).
The first one use connected-component analysis similarly to our proposition and the second is based on growing region.
The performed the evaluation on the 1092 balloons of the eBDtheque dataset~\cite{Guerin2013} at object and pixel levels in order to provide a performance evaluation for localisation and contour analysis purposes (sections~\ref{sub:ex:object_localisation_metric} and~\ref{sub:ex:object_segmentation_metric}).
...
We reused all the rules and heuristics of the original paper except for the straight line detection that we rotated of 90 degrees for images using horizontal text.

The score of this method  was in average for recall and precision of \modif{??\%} and \modif{??\%} (Figure~\ref{fig:ex:balloon_arai_extraction_detail}).
...
The score of this method  was in average for recall and precision of \modif{??\%} and \modif{??\%} (Figure~\ref{fig:ex:balloon_ho_extraction_detail}).
...
The best score of \modif{$R=??$} and \modif{$P=??$} was obtained for \modif{$C_{balloon} > 10\%$} (Figure~\ref{fig:ex:balloon_methodA_extraction_detail}).
...
%We consider as correctly detected the balloon with a confidence value higher than \modif{10\%} (Table~\ref{tab:ex:bal_loc}, Method A (1)).
...
% \extrarowheight as needed to properly center the text within the cells
  \centering
  \caption{Implicit balloon performance evaluation at object and pixel-levels for different energy functions.}
...
%   Ho~\cite{Ho2012}  & ???     & ???   & & ???     & ???&    \\
  ...
  \hline
   $E = E_{int} + E_{ext}$    & \modif{???}       & \modif{???}     & \modif{???}       & \modif{???}  & \modif{???} & \modif{???}   \\
  \hline
  $E = E_{int} + E_{ext} + E_{text}$   & \modif{???}       & \modif{???}     & \modif{???}       & \modif{???}  & \modif{???} & \modif{???}   \\
  \hline
  \end{tabular}
...
The best score of \modif{$R=??$} and \modif{$P=??$} was obtained for \modif{$C_{balloon} > 10\%$}, details are given Figure~\ref{fig:ex:balloon_methodB_extraction_detail}.
...
The ground truth was composed by \modif{68\%}, \modif{18.3\%} and \modif{13.7\%} speech balloons of type ``smooth'', ``wavy'' and ``zigzag'' respectively.
This unbalanced repartition reflects the usual speech balloon repartition in comics.

For the shape/contour separation part (section~\ref{sec:contour_detection}) we first cast the original signal $o$ into 360 values and then we use a window of size $M=20$ according to the parameter validation Table~\ref{tab:validation_parameter_m} (maximum average and global accuracy).
...
First we show the effect of the two features $f_1$ and $f_2$ separately with without a priori information $P(l_1)=P(l_2)=P(l_3)$ and then using the prior probabilities about the classes repartition from the eBDtheque dataset \modif{$P(l_1)=0.68, P(l_2)= 0.183$ and $P(l_3)=0.137$}.
We detailed our best results from ground truth and automatic balloon contour extraction in the confusion matrix Table~\ref{tab:ex:confusion_matrix_balloon_classification}.
...
Tail removal is performed by ignoring the points of the contour that are between the two points defined as tail origin ($f_a$ and $f_b$ in Figure~\ref{fig:se:convexity_defects}).
  ...
  \begin{table}[ht]
    ...
    \centering
    \caption{Classification result accuracy for different descriptor configuration. \modif{TODO: update?}}
    \begin{tabular}{c|c|c|c|c|c|}
  \cline{2-6}
    & \multicolumn{5}{|c|}{Accuracy per class (\%)} \\
  \cline{2-6}
    & Smooth & Wavy & Zigzag & Average & Global \\
  \hline
  \multicolumn{1}{ |c| }{ $D=(f_1)$ } & 86.5 & 48.5 & 70.6  & 68.5 & 77.4\\
  \hline
  \multicolumn{1}{ |c| }{ $D=(f_2)$ } & 89.3 & 69.1 & 86.2 & 81.5 & 85.2\\
  \hline
  \multicolumn{1}{ |c| }{$D=(f_1, f_2)$ } & 89.3 & \textbf{69.1} & \textbf{86.2} & \textbf{81.5} & \textbf{85.2}\\
  %\hline
  \hline
  \multicolumn{1}{ |c| }{$D=(f_1, f_2)$ + $priors$}   & \textbf{93.3} & 55.9 & 82.4 & 77.2 & 84.9\\
  %\hline
  \hline
%       Proposed multi scale & ???  &???  & ???   & ???       \\
%   \hline
...
\renewcommand{\arraystretch}{1.2}
    ...
    \centering
    \caption{Confusion matrix for smooth (S), wavy (W) and zigzag (Z) balloon contour classification results.\modif{TODO: update?}}
    \begin{tabular}{cc|c|c|c|c|c|c|c|c|c|}
  \cline{3-11}
   & & \multicolumn{3}{|c|}{Ground truth} & \multicolumn{3}{|c|}{Auto} & \multicolumn{3}{|c|}{Auto no tail} \\
  \cline{3-11}
   & & S & W  & Z & S & W  & Z & S & W  & Z   \\
  \hline
  \multicolumn{1}{ |c|  }{\multirow{3}{*}{Predict} } & Smooth & \textbf{227} & 17 & 4 & ? & ? & ? & ? & ? & ? \\
  \cline{2-11}
  \multicolumn{1}{ |c  }{} & \multicolumn{1}{ |c| }{ Wavy } & 21  & \textbf{47} & 3  & ? & ? & ? & ? & ? & ?\\
  \cline{2-11}
  \multicolumn{1}{ |c  }{} & \multicolumn{1}{ |c| }{ Zigzag }  & 5 & 4 & \textbf{44}  & ? & ? & ? & ? & ? & ?\\
  %\hline
  \hline
%       Proposed multi scale & ???  &???  & ???   & ???       \\
%   \hline
...
The global accuracy represents the number of good classification divided by the number of element to classify independently from the class.
The confusion matrix table~\ref{tab:ex:confusion_matrix_balloon_classification} shows good classification results for the ``zigzag'' class even for cases that are quite different in terms of drawing styles (Figure~\ref{fig:good_detection_balloon_classification}) and sources (real and synthetic).
...
Also, the tail of the balloon creates some noise and the radius average ($\bar{o}$) we use to normalize the descriptor features may not be appropriate for elongated balloons.
  ...
  \begin{figure}[h]  %trim=l b r t  width=0.5\textwidth,
    ...
    \caption{Correct classification examples for ``zigzag'' (top) and ``smooth'' classes (bottom).}
...
In this section, we will discuss balloon localisation at bounding box and pixel-level, and balloon classification.
...
Balloon localisation aims to evaluate how the presented methods are good at predicting where the balloon are in the image.
Object localisation are performed at the level of object bounding box (Section~\ref{sub:ex:object_localisation_metric}).
Table~\ref{tab:ex:bal_loc} compare the average results we obtained for the best performance of the three proposed methods and two methods from the literature.
...
Method A detects less than half of the balloons (about \modif{$37\%$} recall) of this dataset.
This approach is not originally designed for detecting implicit balloons but knowing the position of the panels allows closing open panels that sometimes recover implicit balloons (Figure~\ref{fig:ex:implicit_balloon_recovery_by_panel}).
...
The results using active contour with distance transform shows a significant improvement, thanks to the active contour theory that detect much more balloons whether open or closed than connected component based methods (Table~\ref{tab:ex:implicit_balloon_performance_object_pixel_comparison}).

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \begin{figure}[h]  %trim=l b r t  width=0.5\textwidth,
    \centering
    \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.75\textwidth]{figure_here.png}
    \caption[Implicit balloon recovering by panel and balloon extraction combination]{Implicit balloon recovering by panel and balloon extraction combination.}
...
For instance, in Method A the performance of our text extractor was \modif{49.75\%} (Table~\ref{tab:text_results}) which was used as input for balloon extraction so the balloon extraction was inevitably affected.
...
As mentioned Section~\ref{??}, the balloon contour contains part of information that is also important to retrieve in the context of understanding comic book documents (e.g. tail, tone information).
...
Such analysis requires a finer extraction of the balloon contour at the level of pixel (Table~\ref{tab:high_res}), not bounding box only.
...
\paragraph{Discussion about implicit balloons \modif{TODO: move?}}
...
In this section we evaluate the tail extraction method on the 1092 balloons of the eBDtheque dataset~\cite{Guerin2013} from two balloon extraction methods, automatic and manual.
The automatic extraction method is the best method from the summary Table~\ref{tab:ex:bal_loc} and the manual is simply the balloon contour from the eBDtheque dataset.
The comparison of both balloon extraction method will highlight the part of error which comes from the balloon extraction and the proper tail extraction method's error when there is not possible error from the balloon extraction.
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|}
          ...
          \hline
          Manual        & \modif{96.77}    & \modif{80.49}     \\
          \hline
          Automatic     & \modif{??}    & \modif{??}     \\
          \hline
...
The few mistakes concerning the tail tip position detection was due to very small tails or when they are partially surrounded by the balloon (Figure~\ref{fig:ex:tail_tip_extraction_mistake_errors}) which confuse the vertex selection form the balloon contour convex hull.
The tail direction was sensitive to the quality of the pixel-level balloon contour extraction which has to be on the external edge to take into account orientation of the closest part to the speaker.
The importance of a precise pixel-level segmentation in the region of the tail tip is illustrated by the difference of \modif{??\%} between the manual and automatic due to the lack of precision of the automatic method.
  ...
  \begin{figure}[h]  %trim=l b r t  width=0.5\textwidth,
    \centering
    \includegraphics[trim= 0px 0px 0px 0px, clip, width=0.75\textwidth]{figure_here.png}
    \caption{Examples of correct and wrong tail tip position detection. \modif{TODO: add direction mistakes as well?}}
    \label{fig:ex:tail_tip_extraction_mistake_errors}
  \end{figure}
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In the eBDtheque ground truth, only 880 (56.8\%) of the character instances are speaking (and 43.3\% are not).
Therefore, we do not expect to reach 100\% recall and precision with the sequential Method A because it is only able to extract characters from the tail (that are speaking).
...
%In the eBDtheque dataset~\cite{Guerin2013}, ??\% of the characters are in an image where there are no  
    ...
    \begin{figure}[ht]%trim=l b r t  width=0.5\textwidth,  
      \centering
      \includegraphics[trim= 0mm 0mm 190mm 0mm, clip, width=0.5\textwidth]{characters_overlap_ratio.png}
      \caption[Example of comic character region prediction]{Example of predicted region considered as true (green) or false (red) positives according to the validation criteria $a_0>0.5$. Thin grey rectangles are the ground truth regions and the red numbers in the top left corner of each ground truth region is the value of the overlapping ratio $a_0$.}
...
Method A computes a region of interest for comic characters from the tail tip position and tail direction inside a panel region (Section~\ref{sec:se:tail_to_character}).
...
The score of this method was in average for recall and precision of \modif{??}\% and \modif{??}\% (Figure~\ref{fig:ex:character_methodA_extraction_detail}).
...
The comic characters extraction are presented in Table~\ref{tab:ex:eval_characters} for the Method A and C.
Method B is evaluated separately on a subset of the dataset.
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          %Proposed (ROI)   & 70   & 49           \\
          Method A (hypothesis + extraction)   & 19.28   & 26.91    & 22.47      \\
          \hline
          ...
          % $a + b + c$  & \modifc{?} & \modifc{?} & ? \\
          % \hline
          Method C (validation)   & \modif{?}   & \modif{?}    & \modif{?}      \\
          \hline
          % TOTAL   & 93.4    & 92.8          \\
          % \hline
        %       Proposed multi scale & ???  &???  & ???   & ???       \\
...
The results present interesting detection results based on a three colours user defined query descriptor (see section~\ref{sec:proposed_method}). 
...
A pre-processing smoothing is required in this case~\cite{l0smoothing2011,Kopf2012DigitalReconstruction}. The colour palette we used has 256 colours, we believe that we can improve the presented method by computing the palette from the user's query in order to ignore unwanted colours and speed up the process. 
...
\subsection{Analysis and comparison} % (fold)
% \label{sub:results_analysis}
