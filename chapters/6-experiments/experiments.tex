...
In this section we evaluate our three approaches and \modif{N} method from the literature on the 850 panels of the eBDtheque dataset~\cite{Guerin2013} ``version 2014'' at bounding box level, and using the recall and precision metrics introduced Section~\ref{sec:ex:metrics}.

\subsection{Experimental settings} % (fold)
\label{sub:experimental_settings}

\subsubsection{Method 1} % (fold)
% \label{sub:method_1}
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          Method A   & \modif{78.02}       & \modif{73.17}   & \modif{75.52}     \\
          \hline
          Method B   & 81.24     & 86.55     & \bf{83.81}      \\
          \hline
          Method C   & 80.69     & 87.03     & 83.74      \\
          \hline
        \end{tabular}
    \label{tab:panel}
  \end{table}%
...
\section{Balloon extraction evaluation (method 1 + 2 + 3)} % (fold)
...
In Section~\ref{sec:se:from_text_to_balloon} we presented two approaches for regular and implicit balloon extraction from text line positions.
...
We consider as correctly detected the balloon with a confidence value higher than \modif{10\%} in Table~\ref{tab:ex:bal_loc} (1).

The implicit balloon extraction method is evaluated using two different scenarios in order to highlight the benefits of both active contour theory
and domain specific knowledge.
% that considers as balloon any white connected components that overlaps at more than $10\%$ with text regions.
First, the active contour with the distance transform based external energy described section~\ref{sec:se:external_energie} (2) and second, we add the domain knowledge energy $E_{text}$ (3) from Section~\ref{sec:se:text_energie}.
% Fourth, after a two stage contour fitting (4).
% Results are presented in table~\ref{tab:ex:bal_loc}.


For both regular and implicit balloon extraction of Method A, the input text is given from the sequential text extraction method (Section~\ref{sec:se:panel_and_text}) as it is the previous step in this approach.
...
%   (2)     & ??? & ??? & ???   & ??? & ??? & ??? \\
  ...
  \hline
  (4)   & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}  & \modif{?}    & \modif{?}  \\
  \hline
...
TODO
  ...
  \begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          % Before validation   & ?   & ?           \\
          \hline
          Other? Arai? Tanaka?\\ Chinese ICDAR? Stroke-based? Lluis?  & \modif{??}   & \modif{??}    & \modif{??}       \\
          \hline
          Method A  & \modif{61.00}   & \modif{19.66}    & \modif{29.75}       \\
          \hline
          Method B  & \modif{?}   & \modif{?}    & \modif{?}       \\
          \hline
...
\subsection{Character spotting} % (fold)
