...
We describe the metrics we used to evaluate each contribution: \modif{Method A, B and C presented \ch{chap:sequential}, \ch{chap:independent} and \ch{chap:knowledge}} respectively and evaluate our work compared to previous works from the literature.

\modif{TODO: introduce short names of the three proposed approaches, rename as Method S, I and KD?}
...
The comic book images were selected to cover the huge diversity of comic styles with the agreement of the consenting authors and publishers that have the objective to foster innovation in this domain trough academic research (Appendix~\ref{app:dataset}).
...
In this section we evaluate our three approaches \modif{(Method A, B and C)} and compare them to two methods from the literature.
...
The score of this method for the independent panel extraction (Method B) was in average for recall and precision of 61.88\% and 87.42\% (Figure~\ref{fig:ex:panel_methodC_extraction_detail}).
...
\begin{figure}[!ht] %trim=l b r t  width=0.5\textwidth, 
  ...
  % \subfloat[Method A]{\label{fig:in:panel_outermost_contours}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=\textwidth,height=4cm\textwidth]{2014-07-20_svg_Arai2010_no_division_linePanel_object.pdf}}
  % \\
  \subfloat[Method C]{\label{fig:ex:panel_methodC_extraction_detail}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=\textwidth]{2014-09-04_cleaned_validated_from_method_B_for_panel_text_balloon_and_method_A_for_character_Panel.pdf}}% \hspace{1em}
  % \\
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|c|}
          ...
          \hline
          Method B          & 62.94       & 87.30   & 73.14      \\
          \hline
          Method C          & 61.88       & \textbf{87.42}     & 72.46      \\
          \hline
        \end{tabular}
...
The drop of Method C can be explained by the fact that the text extractor is also able to detect texts which are not in the speech balloons but the model considers them as noise (rejected).
...
\begin{figure}[!ht] %trim=l b r t  width=0.5\textwidth, 
  ...
  \\
  \subfloat[Method C]{\label{fig:ex:text_methodC_extraction_detail}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=\textwidth]{2014-09-04_cleaned_validated_from_method_B_for_panel_text_balloon_and_method_A_for_character_Line.pdf}}
...
Table~\ref{tab:ex:text_recogniton_results} shows text recognition results given the text line position from the ground truth (best reachable score) and from the best automatic text localisation method (Method B in Table~\ref{tab:text_results}).
...
Considering the best method for balloon extraction (\modif{Method A}), the score of this method was in average for recall and precision of 39.56\% and 48.65\% (Figure~\ref{fig:ex:balloon_methodC_extraction_detail}).
...
% \extrarowheight as needed to properly center the text within the cells
  ...
  \hline
  Method A (regular)& 37.25     & 45.19       & 40.83 & 47.75 & 44.58 & 46.11     \\
  \hline
  ...
  \hline
  Method C (regular)  & 39.55   & \textbf{48.64} & 43.63  & 56.65 & 41.89 & 48.17      \\
  \hline
  \end{tabular}
      \label{tab:ex:balloon_localisation_comparison_results}
\end{table}%
...
The post validation by the expert system (Method C) reached the maximum precision but decreased the recall of the extraction which resulted in a drop of the overall f-measure by almost 5\% compared to Method A.
...
\begin{figure}[!ht] %trim=l b r t  width=0.5\textwidth, 
  ...
  \\
  \subfloat[Method C]{\label{fig:ex:balloon_methodC_extraction_detail}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=\textwidth]{2014-09-04_cleaned_validated_from_method_B_for_panel_text_balloon_and_method_A_for_character_Balloon_object.pdf}}
...
Therefore, we do not expect to reach 100\% recall and precision with the Method A and C because they are only able to extract speaking characters.
The independent Method B is potentially able to retrieve all the character instances of colourful comics, given one example for each of them.
...
\modif{Here we use only the validation part of the expert system without ROI refinement and extra character spotting because these last approaches still under development.}
...
We post processed the output of Method A and we obtained and average score for recall and precision of 3.53\%, 12.50\% and \modif{???}, \modif{???} for automatic and manual previous elements extraction  (Figure~\ref{fig:ex:character_methodC_extraction_detail}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h]
 % \includegraphics[width=\textwidth,height=4cm]{Panel_object_proposed.pdf}
 \centering
  \subfloat[Method C (ground truth) \modif{TODO: update}]{\label{fig:ex:character_method_A_GT_extraction_detail}\includegraphics[trim= 0mm 108mm 0mm 0mm, clip, width=\textwidth]{2014-08-28_svg_method_A_from_GT_panel_balloon_Object.pdf}}
  \\
  \subfloat[Method C (automatic) \modif{TODO: update}]{\label{fig:ex:character_method_A_auto_extraction_detail}\includegraphics[trim= 0mm 0mm 0mm 0mm, clip, width=\textwidth]{2014-08-28_svg_method_A_from_method_B_panel_and_balloon_Object.pdf}}
  \\
 \caption{Character extraction score details using Method C for each image of the eBDtheque dataset.
 }
...
% \extrarowheight as needed to properly center the text within the cells
  ...
  \hline
  Method C (ground truth)  &  \modif{???}  & \modif{???} & \modif{???} &  \modif{???}   &  \modif{???}   & \modif{???}   \\
  \hline
  Method C (automatic)     & \modif{0.0}  & \modif{0.0} & \modif{0.0} &  3.53  & 12.50 & 5.50 \\
  \hline
  \end{tabular}
      \label{tab:ex:character_localisation_result_summary}
\end{table}%
...
Here we take advantage of the independent content extraction method (Method B) and the knowledge base proposed \ch{chap:knowledge}, to evaluate the performance of the expert as the whole, which includes visual and semantic elements.
...
Two high level element retrieval are also evaluated here, the semantic link between the spoken text and speech balloons $STSB$, and also between the speech balloons and speakers $SBSC$.
...
Results are presented Talble~\ref{tab:ex:framework_evaluation_details} and the change in the amount of information discovered throughout the process is presented in Figure~\ref{fig:ex:score_evolution}.
...
%\renewcommand{\arraystretch}{1.2}
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|}
        ...
        Ite. 1 - hypo. & \modif{??}   & \modif{??}  & \modif{??} & \modif{??}  & \modif{??} & \modif{??}  \\
        \hline
        Ite. 1 - valid. + inf. & \modif{??}   & \modif{??}  & \modif{??} & \modif{??}  & \modif{??} & \modif{??}  \\
        \hline
        Ite. 2 - hypo. & \modif{??}   & \modif{??}  & \modif{??} & \modif{??}  & \modif{??} & \modif{??}  \\
        \hline
        Ite. 2 - valid. + inf. & \modif{??}   & \modif{??}  & \modif{??} & \modif{??}  & \modif{??} & \modif{??}  \\
        \hline
        \end{tabular}
...
Note that given the panel, balloon and character position from the ground truth, the accuracy of the expert system in predicting the semantic relations was about $A_{STSB}=96.9\%$ and $A_{SBSC}=70.66\%$.
...
This comparison clearly demonstrate that the expert system is very good at predicting semantic link as soon as the low level extraction are efficient.
...
In this chapter we evaluated our different contributions for content and semantic element retrieval.
...
Then, proposed models combined with independent content extraction methods have been evaluated.

Method A is an intuitive and sequential approach like most of the previously proposed methods in the literature.
It efficiency is maximal when all the image content is present and structured classical comic books.
It gave the best average and precision score for \modif{panel, text, balloon and character extraction} which demonstrate that linking different content extractions at low level processing is relevant for comic book image analysis.

Method B is an independent approach which is more efficient for unusual comic image (no panel, no text or no balloon) than Method A.
However, it is slightly less powerful than Method A in average on the eBDtheque dataset because this dataset is mainly composed by classical comics.
This approach can easily be adapted for particular issues as it does not require previous element extraction.

Method C is able to improve the overall precision but not the recall in the presented configuration because it uses only rejection constraints.
...
\modif{TODO: give details about time processing?}
