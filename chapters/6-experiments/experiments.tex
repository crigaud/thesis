...
We evaluated balloon classification using the pixel-level balloon segmentation from the eBDtheque ground truth and from the best automatic segmentation of regular balloon from Method I.
...
Sometimes, the inside balloon region extraction is not sufficient to retrieve the tail direction, the balloon border has also to be considered (Figure~\ref{fig:ex:tail_extraction_mistake_errors}).
...
The importance of a precise pixel-level segmentation in the region of the tail tip is illustrated by the difference of 7.38\% for $A_{tailDir}$ between the regular and implicit automatic methods due to the lack of precision of the active contour extraction method.
  ...
  \begin{figure}[h]  %trim=l b r t  width=0.5\textwidth,
    ...
    \caption[Examples of difficult balloons for tail tip position and tail direction extractions]{Examples of difficult balloons for tail tip position (left column) and tail direction (right column) extractions. Tail positions are represented by a green line on the balloon contour and tail directions are written in blue (from the eight cardinal coordinates). Left hand side balloons have not been detected without tail because of the contour variations, a tail have been mistakenly predicted. For the right hand side balloons, the tail tip position have been correctly predicted but not the tail direction because of the change of direction of the last segment. The direction have been predicted according the white balloon background region and not from the balloon boundary that indicates a different direction (NE and SE instead of S and E).}
...
We post processed the output of Method S and we obtain an average score for recall and precision of 3.53\%, 12.50\% and \modif{???}, \modif{???} from automatic and ground truth previous elements extraction respectively (Figure~\ref{fig:ex:character_methodC_extraction_detail}).
...
In this chapter we have evaluated our different contributions for content and semantic element retrieval.
...
The sequential approach (Method S) is an intuitive and sequential approach like most of the previously proposed methods in the literature.
It efficiency is maximal when all the image content is present and structured in classical comic books.
It gives the best average and precision score for \modif{panel, text, balloon and character extraction} which demonstrate that linking different content extractions at low level processing is relevant for comic book image analysis.

The independent approach (Method I) is an independent approach which is more efficient for unusual comic image (no panel, no text or no balloon) than Method S.
However, it is slightly less powerful than Method S in average on the eBDtheque dataset because this dataset is mainly composed by classical comics.
This approach can easily be adapted for particular issues as it does not require previous element extraction.

The knowledge-driven approach (Method K) is able to improve the overall precision but not the recall in the presented configuration because it uses only rejection constraints.
...
\modif{TODO: give details about time processing? JCB: yes for the three methods}
