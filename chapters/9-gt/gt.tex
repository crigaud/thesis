...
In the next chapter we are going to propose two methods for panel extraction based on connected component analysis and classification.
% Connected component approaches are very common in scanned stroke-based document analysis, there are also simple and computationally efficient.
...
% In the next chapter we will perform an overall experimental evaluation of all the proposed methods and some of the state-of-the-art algorithms. Although we have already provided experimentations for each of the proposed methods, it is interesting to know which particular methods perform better/worse in which situation. Also a comparison with some state-of-the-art methods is also provided. All the experimentations will be done in symbol spotting framework \ie the performance of the methods will be evaluated depending on the ability of spotting symbols on graphical documents.
